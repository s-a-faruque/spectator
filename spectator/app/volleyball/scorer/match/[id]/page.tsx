...
export default function Match({ params }: { params: Params }) {
  ...
  const [homePlayerName, setHomePlayerName] = useState('Team Blue');
  const [awayPlayerName, setAwayPlayerName] = useState('Team Green');
  const [winningPoint, setWinningPoint] = useState(25);
  const [setNo, setSetNo] = React.useState(1);
  const [previousSets, setPreviousSets] = React.useState<any>([]);
  ...
  useEffect(() => {
    if (id) {
      ...
      if (matchScore) {
        ...
        setPreviousSets(score.previousSets);
        setSetNo(score.setNo);
      }
    }
  }, [id]);
  ...
  const canIncrement = (homeScore: number, awayScore: number) => {
    if(homeScore >= winningPoint && homeScore - awayScore >= 2) {
      setWinner(homePlayerName);
      publishMatchFinished(homePlayerName);
      return false;
    }
    if(awayScore >= winningPoint && awayScore - homeScore >= 2) {
  ...
  const incrementHomePlayerScore = () => {
    ...
    publishScore({setNo, homePlayerScore: homePlayerScore + 1, awayPlayerScore: awayPlayerScore, homePlayerName, awayPlayerName, previousSets});
  ...
  const incrementAwayPlayerScore = () => {
    ...
    publishScore({setNo, homePlayerScore: homePlayerScore, awayPlayerScore: awayPlayerScore + 1, homePlayerName, awayPlayerName, previousSets});
  ...
  const handleClearScore = () => {
    ...
    publishScore({setNo, homePlayerScore: 0, awayPlayerScore: 0, homePlayerName, awayPlayerName});
  }
  ...
  const startNextSet = () => {
    ...
    setPreviousSets([...previousSets, justFinishedSet]);
    ...
    publishScore({
      ...
      homePlayerName,
      awayPlayerName,
      previousSets: previousSets  // Add safely
    });
  ...
  const undoScore = () => {
    if (scoreHistory.length > 0) {
      if (scoreHistory.length === 1) {
        ...
        publishScore({setNo, homePlayerScore: 0, awayPlayerScore: 0, homePlayerName, awayPlayerName, previousSets});
      } else {
        ...
        publishScore({setNo, homePlayerScore: lastScore.home, awayPlayerScore: lastScore.away, homePlayerName, awayPlayerName, previousSets});
      }
  ...
  const handleHomePlayerNameChange = (newName: string) => {
    ...
    publishScore({setNo, homePlayerScore, awayPlayerScore, homePlayerName: newName, awayPlayerName, previousSets});
  }

  const handleAwayPlayerNameChange = (newName: string) => {
    setAwayPlayerName(newName);
    publishScore({setNo, homePlayerScore, awayPlayerScore, homePlayerName, awayPlayerName: newName, previousSets});
  }
    ...
    // <AblyProvider client={ably}>
      <main className={styles.main}>
        <header className={styles.header}>
          <h1>
            <Link className={styles.home} href="/volleyball/scorer">
                <Image
                  src="/homepage.png"
                  alt="Home Icon"
                  width={16}
        ...
        <div className={styles.primaryContent}>
          <div className={styles.scoreboard}>
            ...
            <div className={styles.player}>
              <div className={styles.playerName}>
                <EditableLabel value={homePlayerName} onChange={handleHomePlayerNameChange} />
              </div>
              <div className={styles.playerScore}>{homePlayerScore}</div>
              {previousSets.length > 0 && (
                previousSets.map((set: SetScore) => (
                  <div key={set.setNo} className={styles.playerScore}>
                    {set.homePlayerScore}
                  </div>
                ))
              )}
          ...
          {matchFinished ? (
            <div className={styles.congratulations}>
              <button className={`${styles.undoControlButton} ${nunito.className}`} onClick={startNextSet}>
                  NextSet
                </button>
              <Image
                    src="/medal.png"
